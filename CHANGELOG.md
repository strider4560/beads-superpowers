...
- **A version mismatch between the plugin cache and the install root now denies with a remedy instead of bricking the session.** `hooks/pipeline-guard` compares its own version against the one in the integrity record before it sources the attested `scripts/pipeline/bundle-root.sh`, and a mismatch stops the call naming both versions and the command that clears it — `claude plugin update beads-superpowers` on the plugin channel, or re-running `install.sh` on a scripted install. Until now two halves from different releases failed at whatever the newer library no longer defined: a 0.18.0 plugin cache against a post-rework install root killed every Bash, Write and Edit call in the session with a bash `resolve_session_tier: command not found` and nothing actionable in the message. The check stands down when either side carries no version, so an install predating it is never denied, and a git checkout is exempt outright — a development clone is meant to run ahead of the installed root, which is exactly what the documented maintainer workflow of symlinking the plugin cache at the repo produces. It can only report skew from the side that carries it, so 0.18.0 caches stay silent.

- **`tier-gate.sh --phase` — agent-run pipeline orientation.** Read-only and advisory: it reads the bead graph (open initiatives, epic/task rollups, epics whose tasks are all closed and so await review, open `needs-planning` beads — and, with nothing in flight, the newest spec and its planning-review artifacts) and prints `phase: <planning-needed|planning|capturing|implementing|reviewing|idle>` with evidence lines and a one-line `next:` routing recommendation. `getting-up-to-speed` now fires it during orientation ("where are we", "what's next", "continue where we left off") and carries its `phase:`/`next:` lines into the current-state summary — the user never runs pipeline scripts; the orchestrating agent does. Only the `phase:` and `next:` prefixes are contract. (bead beads-superpowers-bag)

### Changed

- **The great_cto floor moves to 3.2.0.** `GREAT_CTO_MIN_VERSION` now requires the great_cto release that adopted the agent-authority rework. Below it the lifecycle skills still route on `tier-gate.sh --assert` and the exit-4 SKIP, and the rework below removed both, so an older great_cto hands an agent a flag that no longer exists. Update great_cto first — a bundle root below the floor refuses every stage with the reinstall command rather than running degraded.
...
- **BREAKING: [great_cto](https://github.com/strider4560/great_cto) is now a hard dependency, and the floor is version 3.1.0.** Every pipeline stage resolves the bundle root at `~/.agents/great_cto` and reads its version; a missing root, or one below 3.1.0, fails closed with the install-or-update command and stops the skill. There is no degraded standalone mode and no override. great_cto 3.1.0 is the matching rework release, and 0.18.0 ships only after it does — the version floor is the only thing enforcing that ordering, and an older great_cto would hand `brainstorming` back to a wrapper-era `planning-with-reviews` and recurse. Update great_cto first: a root below the floor, or one predating great_cto's own version file, refuses every stage with the re-install command rather than running degraded. That is the hard dependency behaving as designed, not a defect to route around. (ADR-0061)
- **Work runs as a five-stage pipeline split across two sessions.** A planning session runs `brainstorming` → great_cto's `planning-with-reviews` → `writing-plans`; an implementation session runs great_cto's `implementing-epics` → `reviewing-epics`. `using-superpowers` routes "let's build X" to the first and "implement the ready epic" to the second, each stage checks its tier before it starts, and a design question that surfaces mid-implementation files a `needs-planning` bead instead of re-planning in lane.
- **The bead graph and `.mex/` are the plan of record.** `writing-plans` now persists the plan itself — one initiative epic labeled `initiative`, its child epics carrying reviewer rosters in `metadata`, and one task bead per task, imported as JSONL — then records three `mex log --type decision` lines and a distilled block in `.mex/context/initiatives.md`, and lints the result before it hands off. Nothing is written to a machine-readable plan file and nothing is imported at implementation time: the implementation session claims beads that already exist. `writing-plans` ends by reporting the initiative id and stopping, because a planning-tier session that carries on into implementation is the tier wall failing open.
- **`subagent-driven-development` became the pipeline's task engine.** A caller hands it task groups — beads that share one implementation role and touch no common path, in a worktree the caller already created — and it produces one brief, one implementer subagent, and one review per group, reporting verdicts per bead id. It no longer creates or closes beads, parses plan files, merges branches, or removes worktrees; the caller owns all of that. `implementing-epics` is the normal caller, and the user can still direct in-session execution after `writing-plans` presents its handoff, in which case the planning agent holds both jobs. One consequence to know: on the `implementing-epics` path the engine reports that no whole-branch review has run and leaves the decision to its caller, so whether the composite of all groups and all fix rounds gets reviewed now depends on great_cto.

### Added

- **Three gates enforce the tier and graph rules in code.** `scripts/pipeline/tier-gate.sh` refuses stage entry from the wrong tier — exit 1 fails closed with no override, and exit 4 is a visible SKIP on a harness that cannot report its session model, which the caller must resolve by asking the user for explicit confirmation, never by continuing quietly. `scripts/pipeline/graph-lint.mjs` validates the captured bead graph before an implementation session consumes it, and reports the epic and task counts so an import that silently landed nothing cannot read as a clean graph. `hooks/pipeline-guard`, a PreToolUse deny hook, blocks plan-graph mutation from a non-planning session, source writes from a planning session, self-granted tiers, and any write into the pipeline's state directory. The guard stays inert until a project has both the bundle root and a tier map, so projects that never use the pipeline pay nothing for it. `install.sh` populates an install root at `~/.agents/beads-superpowers` with the gate scripts and the guard, and registers the guard as a `PreToolUse` hook over `Bash|Write|Edit|NotebookEdit`, so the gates reach an installed user and not just a clone. great_cto's stage skills resolve all three contract paths — `tier-gate.sh`, `graph-lint.mjs`, and `review-package` — through that root and never through a path relative to the working project, because a working project that supplies its own gate is a working project that passes itself.

### Fixed

- **A tier that declares no session effort no longer makes its stage unenterable.** The gate read an absent `session_effort` as the literal `"default"` and then required the session to match it — but a session reports `low`, `medium`, `high`, `xhigh`, or `max`, never `default`, so any tier that omitted the key refused every session that can see its own effort. A tier that sets `session_effort` to `default` failed the same way. Both now mean what they read as: this tier constrains effort not at all. A tier naming a real effort still enforces it.
- **A denied tool call says why it was denied.** When the guard cannot resolve the session's tier — the session state names a model the tier map does not list, or the state file is not valid JSON — it now carries that reason, and the remedy, into its one-line deny. It previously reported only that resolution failed, which left every `Bash`, `Write`, and `Edit` call in the session refused with nothing to act on, and the `--assert` remedy named elsewhere does not clear this particular state.
- **The hook test suites no longer overwrite the pipeline's own session state.** Two of them ran the session-start hook with the real `$HOME` and no working directory of their own; on a machine with the pipeline armed, that wrote over the live session state once per hook invocation. Both now run from a scratch workspace under a scratch `$HOME`, and the dialect suite asserts the repo's own state file is byte-identical after the run.

### Deprecated

- **`executing-plans` is deprecated.** Plan execution goes through the pipeline instead: `writing-plans` captures the bead graph and hands off to `implementing-epics`, which forms the task groups and invokes `subagent-driven-development`. The skill is reduced to a pointer so existing references still resolve, and its load-review-execute loop, blocker taxonomy, and epic/task walkthrough stay in git history. The sweep behind it is incomplete — the README, `CLAUDE.md`'s skills table, and the docs-site skills and tips pages in both languages still list `executing-plans` as a live execution option.
...
- **STE authoring rules for machine-parsed text** (fork addition). A condensed Simplified Technical English rule set — adapted from [danyuchn/asd-ste100-skill](https://github.com/danyuchn/asd-ste100-skill) (MIT), itself encoding ASD-STE100 Issue 9 — now governs every string one agent writes for another agent to parse: bead descriptions and acceptance criteria (`writing-plans`), scene-setting context, implementer reports, review findings, close reasons, and the durable-insight entries written to `.mex/lessons.md` (`subagent-driven-development`), handoff docs (`session-handoff`), and spec requirement lines (`brainstorming`). Canonical reference: `skills/subagent-driven-development/references/ste-authoring.md`. Rationale: a fresh subagent reads its brief cold; a sentence with two readings costs a fix round, and synonym rotation across parallel briefs makes independent implementers diverge on shared names.

### Changed

- **`mex-curator` replaces `memory-curator`.** The new skill distills a session's durables into `.mex/` — routing each item to its page, deduping against what is already there, enforcing the 2 KB hot-page cap by demoting the coldest entries to `.mex/lessons-archive.md`, and superseding stale entries. Like its predecessor it proposes an edit list you approve before anything is written, and it never deletes first. `finishing-a-development-branch` offers a curation pass before the push when a session captured roughly three or more durables, or whenever session start showed the hot-page truncation marker. Retired `memory-curator` directories are removed from every harness skill directory on upgrade and uninstall.

### Removed

- **Knowledge-beads and the `bd remember` memory store are retired.** `bd remember` / `bd memories` / `bd recall` / `bd forget` and the `kb`-labelled deferred knowledge-beads are gone from every skill, the hook, and the example workflow; `scripts/check-no-legacy-memory-refs.sh` fails the build if any of them reappear. `project-init` carries a **Migrating from knowledge-beads** section — the one allowlisted site that may still name the retired commands, because it exists to retire them — with the procedure for moving an existing store into `.mex/`.
...
- **BREAKING: OpenCode install is now git-install only, exactly like upstream.** The plugin moved to `.opencode/plugins/beads-superpowers.js` (upstream's plugin as the base — message-transform bootstrap, config-hook skill auto-registration — plus the beads graft: composer-sourced context and compaction re-injection). Install by adding `"plugin": ["beads-superpowers@git+https://github.com/strider4560/beads-superpowers.git"]` to opencode.json; `package.json` gains the `main` entry the git spec loads. `install.sh` no longer copies OpenCode artifacts — pre-0.12 copies keep working but are frozen; migrate per `.opencode/INSTALL.md` (or `install.sh --uninstall` removes them). The version-sync set drops `opencode/package.json` (8 → 7 files).
...
- npx installation method via Vercel Skills CLI (`npx skills add strider4560/beads-superpowers`)
