...
| The full `bd prime` dump costs far more injected context than a curated one (measured on this project: -91.6% on a 218-memory store) | Session hook injects only the latest continuation memory under a byte cap; the salience-curated digest itself now surfaces on demand | `session-start` hook, `getting-up-to-speed` |
