...
**按显著度精选的上下文注入。** 当时，插件的 session-start 钩子组合出一份 beads 上下文（精选记忆加上一个 `bd prime` 指引），而不是注入完整的 `bd prime` 转储。在一个包含 218 条记忆的库上测量，精选版本将注入上下文削减了 91.6%。正是这项测量促成了钩子对注入内容设置上限，而非直接转储整个存储。此后设计又更进一步：持久知识从 beads 迁往 `.mex/` 存储，钩子现在在受字节预算限制的信封内注入一行路由指引加上 2 KB 的经验热页面，其余一切都在检索时按需路由。
...
| 完整的 `bd prime` 转储所占用的注入上下文远多于精选版本（本项目实测：在 218 条记忆的库上减少 91.6%） | 会话钩子在字节上限内注入一行路由指引加上 2 KB 热页面；其余每个页面都按需呈现 | `session-start` 钩子、`mex-curator` |
