...
Route on the answer. **Record the decision / Both** → this writes an ADR, so first confirm it clears the bar (hard-to-reverse AND surprising-without-context AND genuine trade-off); if it doesn't, say so and capture it via `mex log` alone (the lighter record) — unless the user confirms they want the full ADR. Write the ADR (`docs/decisions/ADR-NNNN-<kebab>.md`, sections Context/Decision/Rationale/Consequences, update `docs/decisions/INDEX.md`), then run `mex log "<one-line summary> — see docs/decisions/ADR-NNNN-<kebab>.md"` (run the secret/PII scan on the summary first). A superseding ADR's log line must read `supersedes ADR-NNNN`, and `mex-curator` updates `.mex/decisions.md` so the stale entry points forward. **Remember the lesson / Both** → append to `.mex/lessons.md` per the capture contract. **Skip** → nothing.
