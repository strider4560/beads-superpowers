#!/usr/bin/env bash
# test-find-polluter.sh — deterministic guard for skills/systematic-debugging/find-polluter.sh.
# Ported from upstream superpowers v6.2.0 tests/systematic-debugging/test-find-polluter.sh
# (issues #2008, #2011). Guards the false-clean verdict: a pattern that matches nothing
# must report 0, never 1, and the documented pattern must match both nested and
